...
if (Config.WORKTYPE == 'private') {
	...
	try {
	  ...
	  if (response.statusCode === 200) return await message.client.sendMessage(message.jid, ' \n\n*Vidhya* 😘' + Lang.BOT +' ```' + json.messages[0].response + '```\n\n' , MessageType.text,{quoted: message.data});
    ...
    Asena.addCommand({pattern: 'sophia ?(.*)', fromMe: false, desc: Lang.BOT_DESC}, async (message, match) => {
	...
	try {
	  ...
	  if (response.statusCode === 200) return await message.client.sendMessage(message.jid, ' \n\n*Vidhyamwol* 😘' + Lang.BOT +' ```' + json.messages[0].response + '```\n\n' , MessageType.text,{quoted: message.data});
